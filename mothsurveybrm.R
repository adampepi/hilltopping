...
library(cowplot)
library(performance)
setwd("~/Documents/Research/dissertation/12 sites analyses/12sitesanalyses/hilltopping")
...
mothsCI$year<-as.factor(mothsCI$year)
...
mci1<-brm(count~CIS+(1|site)+(1|year),data=mothsCI,family='poisson',prior=prior1)

summary(mci1)


mci2<-brm(count~CIS+elevation+(1|site)+(1|year),data=mothsCI,family='poisson',prior=prior1)
summary(mci2)
mci3<-brm(count~CIS*elevation+(1|site)+(1|year),data=mothsCI,family='poisson',prior=prior1)
summary(mci3)
mci4<-brm(count~elevation+(1|site)+(1|year),data=mothsCI,family='poisson',prior=prior1)
summary(mci4)
mci5<-brm(count~(1|site)+(1|year),data=mothsCI,family='poisson',prior=prior2)
summary(mci4)
...
summary(mci2)
posterior_interval(mci2,prob=0.95)
posterior_interval(mci2,prob=0.90)
r2_bayes(mci2)
...
elevationplot<-mothsCI %>%
  ...
  scale_color_brewer(palette = "Dark2")+theme_classic()+scale_y_log10()+xlab('Elevation, m')+ylab("Moth count")
...
catCI$year<-as.factor(catCI$Ciyear)

catCI$year

mcci1<-brm(logcount~CIS+(1|site)+(1|year),data=catCI,iter=10000, prior=prior1)
summary(mcci1)
...
mcci1.1<-brm(logcount~(1|site)+(1|year),data=catCI,iter=10000, prior=prior2)
summary(mcci1.1)
...
ggplot(catCI,aes(x=CIS,y=logcount,color=year,label=year))+geom_text(size=3)+geom_smooth(method='lm')
...
connplot2<-catCI %>%
  ...
  geom_line(aes(y = .value,group = paste(.draw)), alpha = .1) +
  stat_lineribbon(aes(y = .value),.width = c(0, 0, 0),show.legend = F)+
  geom_point(data = catCI) +theme_classic()+xlab('Connectivity')+ylab("ln(Caterpillars/lupine+1)")
connplot2
